...
void performHealthCheck() {
  ...
  if (freeHeap < CRITICAL_HEAP_THRESHOLD) {
    DEBUG_PRINTF("[HEALTH] KRITIK! Bellek: %lu bytes - Restart gerekli!\n", freeHeap);
    delay(100);
    ESP.restart();
  }
  ...
  if (freeHeap < WARNING_HEAP_THRESHOLD) {
    DEBUG_PRINTF("[HEALTH] UYARI! Düsük bellek: %lu bytes (%d%%)\n", freeHeap, heapPercent);
  }
  ...
  if (wifi.isConnectedToWiFi()) {
    ...
    if (wifiDownSince == 0) {
      ...
      DEBUG_PRINTLN("[HEALTH] WiFi baglantisi kayboldu, izleniyor...");
    } else if (millis() - wifiDownSince > CRITICAL_WIFI_DOWN_TIME) {
      DEBUG_PRINTF("[HEALTH] WiFi %lu dakikadir bagli degil - Restart!\n", 
  ...
  // Periyodik durum logu (her 5 dakikada bir)
  static unsigned long lastStatusLog = 0;
  if (millis() - lastStatusLog > 300000) {
    lastStatusLog = millis();
    DEBUG_PRINTF("[HEALTH] Heap: %lu/%lu (%d%%), Min: %lu, Uptime: %lu dk\n",
                 freeHeap, totalHeap, heapPercent, minFreeHeap, millis() / 60000);
  }
}
...
void loop() {
  ...
  // OTA otomatik güncelleme kontrolü (restart'a dayanıklı)
  if (wifi.isConnectedToWiFi()) {
    // İlk çalışmada kalan süreyi hesapla (restart sonrası devam eder)
    if (otaCheckInterval == 0) {
      otaCheckInterval = getRemainingOTATime();
      lastOTACheck = millis();
    }
    
    // Süre dolduysa kontrol et (yeni random süre autoUpdateCheck içinde planlanır)
    if (millis() - lastOTACheck > otaCheckInterval) {
      lastOTACheck = millis();
      autoUpdateCheck();  // Bu fonksiyon içinde scheduleNextOTACheck() çağrılır
      otaCheckInterval = getRemainingOTATime();
    }
  }
