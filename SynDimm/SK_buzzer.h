...
public:
    void begin() {
        ...
        Serial.println("[Buzzer] Initialized on GPIO 17 (2kHz PWM)");
    }
    ...
    void playDits(int count) {
        if (count < 1 || count > 3) {
            Serial.printf("[Buzzer] Invalid dit count: %d\n", count);
            return;
        }
        
        if (buzzerState != BUZZER_IDLE) {
            Serial.println("[Buzzer] Busy - ignoring new dit request");
            return;
        }
        
        Serial.printf("[Buzzer] Playing %d dit(s)\n", count);
